...
import { watchAdComplete, getUserHistory } from '../services/userService';
...
export default function Earn() {
  ...
  const handleWatchAd = async () => {
    ...
    setAdLoading(true);
    try {
      const adSuccess = await showRewardedPopup(zoneId, telegramId.toString());
      if (adSuccess) {
                ...
                <div key={item._id} className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/5 transition-all">
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-[11px] text-gray-400">
                      {item.createdAt ? new Date(item.createdAt).toLocaleDateString() : ''} {item.createdAt ? new Date(item.createdAt).toLocaleDateString([], {hour: '2-digit', minute:'2-digit'}) : ''}
                    </span>
                  </div>
